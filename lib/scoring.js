...
function computeDiscoverScore({
  ...
  else if (htmlResult.undeclaredRssUrl) score += 2;
...
function generateRecommendations({ robotsResult, llmsResult, htmlResult, sitemapResult, coreWebVitals, authorInsights }) {
  ...
  if (!htmlResult.rssHref) {
    if (htmlResult.undeclaredRssUrl) {
      recs.push({
        priority: 'low',
        text: `An RSS feed exists at ${htmlResult.undeclaredRssUrl} but isn't declared with a <link rel="alternate"> tag in the head — add one so feed readers and tools can auto-discover it.`,
      });
    } else {
